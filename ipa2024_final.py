...
def process_message(message_text, student_id):
    ...
    if command == "create":
        ...
        if status == "exists_enabled":
            return ('text', f"Interface loopback {student_id} is enabled") # <--- แก้ไข
        elif status == "exists_disabled":
            return ('text', f"Interface loopback {student_id} is disabled") # <--- แก้ไข
        elif status == "not_exists":
            return ('text', f"No Interface loopback {student_id}") # <--- แก้ไข
        else:
            return ('error', "Error checking status.") # <--- แก้ไข (เป็น 'error')
    
    # --- 2. ส่วนของ Netmiko และ Ansible (เหมือนเดิม) ---
    elif command == "gigabit_status":
        msg = netmiko_final.gigabit_status(ROUTER_IP)
        if msg.startswith("Error:"): # <--- (แถม) ทำให้ดีขึ้น
             return ('error', msg)
        else:
             return ('text', msg)
